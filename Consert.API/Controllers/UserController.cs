...
namespace Consert.API.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        {
            var user = await _unitOfWork.GetUserByEmailAsync(email);
        ...
        public async Task <IActionResult> AddUser([FromBody] UserDto userDto)
        {
            ...
            await _unitOfWork.AddUserAsync(user);
            return Ok();
        }
        [HttpPut("{id}")]
        ...
        {
            var user = _mapper.Map<User>(userDto);
            _unitOfWork.UpdateUserAsync(user);
            return Ok();
        }
        [HttpDelete("{id}")]
        public async Task <IActionResult> DeleteUser(string id)
        {   var user = await _unitOfWork.GetUserByIdAsync(id);
            await _unitOfWork.DeleteUserAsync(user);
            return Ok();
        }
    }
}